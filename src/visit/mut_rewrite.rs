...
pub trait Rewriter<'a>: Sized {
    fn visit_root(&mut self, s: &'a Root) -> Root {
    ...
    fn enter_root(&mut self, s: &'a Root) -> Option<Root> {
        None
    }

    fn leave_root(&mut self, s: Root) -> Root {
        s
    }

    fn visit_cmd(&mut self, s: &'a Cmd) -> Cmd {
    ...
    fn enter_cmd(&mut self, s: &'a Cmd) -> Option<Cmd> {
        None
    }

    fn leave_cmd(&mut self, s: Cmd) -> Cmd {
        s
    }

    fn visit_def(&mut self, s: &'a Def) -> Def {
    ...
    fn enter_def(&mut self, s: &'a Def) -> Option<Def> {
        None
    }

    fn leave_def(&mut self, s: Def) -> Def {
        s
    }

    fn visit_exp(&mut self, s: &'a Exp) -> Exp {
    ...
    fn enter_exp(&mut self, s: &'a Exp) -> Option<Exp> {
        None
    }

    fn leave_exp(&mut self, s: Exp) -> Exp {
        s
    }

    fn visit_param(&mut self, s: &'a Param) -> Param {
    ...
    fn enter_param(&mut self, s: &'a Param) -> Option<Param> {
        None
    }

    fn leave_param(&mut self, s: Param) -> Param {
        s
    }

    fn walk_root(&mut self, s: &'a Root) -> Root {
        match *s {
    ...
    fn walk_cmd(&mut self, s: &'a Cmd) -> Cmd {
        match *s {
    ...
    fn walk_def(&mut self, s: &'a Def) -> Def {
        match *s {
    ...
    fn walk_exp(&mut self, s: &'a Exp) -> Exp {
        match *s {
    ...
    fn walk_param(&mut self, s: &'a Param) -> Param {
        match *s {
